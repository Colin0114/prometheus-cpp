...
Gateway::Gateway(const std::string& host, const std::string& service, const std::string jobname,
                 const Labels& labels, const std::string username,
                 const std::string password) : host_{host}, service_{service} {
...
std::future<int> Gateway::async_push(HttpMethod method) {
  ...
  for (auto& wcollectable : collectables_) {
    ...
    futures.push_back(std::async(std::launch::async, [&] {
      return performHttpRequest(method, target, body);
    }));
  }
